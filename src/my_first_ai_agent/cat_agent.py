...
from my_first_ai_agent.tools import read_s3_file, write_s3_file, web_fetch
...
def run_cat_agent():
    cat_agent = create_agent(
        ...
        tools=[read_s3_file, write_s3_file, web_fetch, send_message_to_telegram_bot],
        system_prompt=f"""Your are a good assistant to help seek new cats from a shelter website
        
        all file operations are limited in s3 bucket: 'yanbin-data', s3key: 'ai-agent/ai-agent/found_cat_ids.txt'
        """
    )
    ...
    Use s3 file bucket: 'yanbin-data', s3key: 'ai-agent/found_cat_ids.txt' to store Cat IDs and determine if new cat.
